...
function renderGroundDebug() {
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(sonic.x - 100, groundLevel);
    ctx.lineTo(sonic.x + sonic.width + 100, groundLevel);
    ctx.stroke();
    
    // Draw continuous ground profile
    ctx.strokeStyle = 'rgba(0, 255, 0, 0.8)';
    ctx.lineWidth = 2;
    ctx.beginPath();
    
    const startX = Math.max(0, camera.x - 100);
    const endX = camera.x + canvas.width + 100;
    
    let firstPoint = true;
    for (let x = startX; x < endX; x += 10) {
        const groundY = getGroundLevel(x);
    ...
    for (let i = -1; i < 6; i++) {
        const tileX = (tileIndex + i) * 320;
        const tileScreenX = tileX - camera.x;
        
        if (tileScreenX > -50 && tileScreenX < canvas.width + 50) {
            const tileType = (tileIndex + i) % 3;
            const typeNames = ['T1-FLAT', 'T2-CLIFF', 'T3-HILL'];
            
            ctx.strokeStyle = 'rgba(0, 255, 255, 0.6)';
            ctx.lineWidth = 1;
            ctx.beginPath();
            ctx.moveTo(tileScreenX, 300);
            ctx.lineTo(tileScreenX, 550);
            ctx.stroke();
            
            ctx.fillStyle = 'cyan';
            ctx.font = '10px Arial';
            ctx.fillText(`${typeNames[tileType]}`, tileScreenX + 5, 315);
        }
    }
    ...
    const typeNames = ['T1-FLAT(505)', 'T2-CLIFF(370→425)', 'T3-HILL(500→425)'];
    ctx.fillText(`Tile: ${tileIndex} | ${typeNames[terrainType]} | Pos: ${Math.floor(positionInTile)} | Ground: ${Math.floor(groundLevel)}`, sonic.x, sonic.y - 40);
    ctx.fillText(`OnGround: ${sonic.onGround} | VelY: ${sonic.velocityY.toFixed(1)}`, sonic.x, sonic.y - 25);
    
    // Sonic collision box
    ctx.strokeStyle = sonic.onGround ? 'rgba(0, 255, 0, 0.8)' : 'rgba(255, 0, 0, 0.8)';
    ctx.lineWidth = 2;
    ctx.strokeRect(sonic.x, sonic.y, sonic.width, sonic.height);
}
